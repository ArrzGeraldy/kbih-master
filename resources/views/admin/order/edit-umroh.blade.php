        ...
        <main class="flex-1 overflow-y-auto p-6">
          <div class="max-w-4xl">
            <div class="flex items-start justify-between gap-4 mb-6">
              <div>
                <h2 class="text-xl font-semibold text-gray-900">Edit Detail Umroh</h2>
                <p class="text-sm text-gray-600">Perbarui tanggal keberangkatan dan kelas bimbingan untuk order ini.</p>
              </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-6">
              <form method="POST" action="{{ route('admin.orders.update-umroh', $order) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-6  ">
                  ...
                  <div class="">
                    ...
                    <input
                      ...
                      value="{{ old('tanggal_keberangkatan', optional($order->orderUmrohDetail)->tanggal_keberangkatan?->format('Y-m-d')) }}"
                  ...
                  <div class="">
                    ...
                    <select
                      ...
                      class="mt-1 block w-full rounded-md border-gray-300 focus:border-green-900 focus:ring-green-900"
                    >
                      <option value="" disabled {{ old('kelas_bimbingan_id', optional($order->orderUmrohDetail)->kelas_bimbingan_id) ? '' : 'selected' }}>
                        Pilih kelas bimbingan
                      </option>
                      @foreach ($kelasBimbingans as $kelas)
                        <option
                          value="{{ $kelas->id }}"
                          @selected(old('kelas_bimbingan_id', optional($order->orderUmrohDetail)->kelas_bimbingan_id) == $kelas->id)
                        >
